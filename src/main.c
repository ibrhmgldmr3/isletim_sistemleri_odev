...
#define MAX_ARKA_PLAN_SUREC 10 // Aynı anda çalışabilecek maksimum arka plan süreç sayısı

/*
 * Fonksiyon: handle_sigchld
 * Açıklama: SIGCHLD sinyali için işleyici. Arka plan süreçleri sona erdiğinde çağrılır.
 * Parametreler:
 *   - sig: Sinyal numarası (kullanılmıyor).
 */
void handle_sigchld(int sig);

/*
 * Fonksiyon: print_prompt
 * Açıklama: Kabuk prompt'unu ekrana yazdırır.
 */
void print_prompt();

/*
 * Fonksiyon: execute_single_command
 * Açıklama: Tek bir komutu çalıştırır.
 * Parametreler:
 *   - cmdLine: Çalıştırılacak komut satırı.
 */
void execute_single_command(char *cmdLine);

/*
 * Fonksiyon: execute_command_with_input_redirection
 * Açıklama: Giriş yönlendirmesi (<) içeren bir komutu çalıştırır.
 * Parametreler:
 *   - cmd: Çalıştırılacak komut ve giriş dosyasını içeren komut satırı.
 */
void execute_command_with_input_redirection(char *cmd);

/*
 * Fonksiyon: execute_command_with_output_redirection
 * Açıklama: Çıkış yönlendirmesi (>, >>) içeren bir komutu çalıştırır.
 * Parametreler:
 *   - args: Komut argümanlarını içeren dizi.
 *   - output_file: Çıkışın yönlendirileceği dosya adı.
 *   - append_flag: Ekleme modunda (>>) açılıp açılmayacağını belirten bayrak.
 */
void execute_command_with_output_redirection(char **args, char *output_file, int append_flag);

/*
 * Fonksiyon: parse_and_execute_output_redirection
 * Açıklama: Komut satırını ayrıştırır ve çıkış yönlendirmesi (>, >>) varsa işler.
 * Parametreler:
 *   - input: Ayrıştırılacak komut satırı.
 */
void parse_and_execute_output_redirection(char *input);

/*
 * Fonksiyon: arka_plan_surec_ekle
 * Açıklama: Arka plan süreç listesine yeni bir süreç ekler.
 * Parametreler:
 *   - pid: Süreç ID'si.
 *   - komut: Sürecin komut satırı.
 */
void arka_plan_surec_ekle(pid_t pid, const char* komut);

/*
 * Fonksiyon: arka_plan_surec_kaldir
 * Açıklama: Arka plan süreç listesinden bir süreci kaldırır.
 * Parametreler:
 *   - pid: Kaldırılacak sürecin ID'si.
 */
void arka_plan_surec_kaldir(pid_t pid);

/*
 * Fonksiyon: arka_plan_surecleri_kontrol
 * Açıklama: Arka plan süreçlerini kontrol eder ve bitenler hakkında bilgi verir.
 */
void arka_plan_surecleri_kontrol();

/*
 * Fonksiyon: execute_with_pipe
 * Açıklama: Boru (|) içeren bir komut satırını çalıştırır.
 * Parametreler:
 *   - input_buffer: Çalıştırılacak komut satırı.
 */
void execute_with_pipe(char *input_buffer);

/*
 * Fonksiyon: list_jobs
 * Açıklama: Arka planda çalışan süreçleri listeler.
 */
void list_jobs();

/*
 * Fonksiyon: increment_command
 * Açıklama: "increment" komutunu işler. Standart girdiden bir tamsayı okur,
 *            bir arttırır ve sonucu standart çıktıya yazar.
 */
void increment_command();

/*
 * Fonksiyon: execute_multiple_commands
 * Açıklama: Noktalı virgül (;) ile ayrılmış birden fazla komutu çalıştırır.
 * Parametreler:
 *   - input_buffer: Çalıştırılacak komut satırı.
 */
